...
async function main() {
  ...
  if (!NEW_TIME26 || !NEW_PROOF_RECORDER) {
    console.error("❌ ERROR: Please update NEW_TIME26 and NEW_PROOF_RECORDER addresses!");
  ...
  console.log("   Deployer balance:", ethers.utils.formatUnits(deployerBalance, 18), "TIME26");
  ...
  await tx.wait();
  console.log("   ✅ Transferred", ethers.utils.formatUnits(REWARD_POOL, 18), "TIME26 to ProofRecorder");

  // Transfer to Treasury
  console.log("   Transferring to Treasury...");
  tx = await time26.transfer(TREASURY_SAFE, TREASURY_AMOUNT);
  await tx.wait();
  console.log("   ✅ Transferred", ethers.utils.formatUnits(TREASURY_AMOUNT, 18), "TIME26 to Treasury");
  ...
  console.log("ProofRecorder balance:", ethers.utils.formatUnits(recorderBalance, 18), "TIME26");
  console.log("Treasury balance:", ethers.utils.formatUnits(treasuryBalance, 18), "TIME26");
  console.log("Deployer remaining:", ethers.utils.formatUnits(remainingBalance, 18), "TIME26");
  console.log("");
  ...
  console.log("");
  console.log("2. Verify contracts on PolygonScan (commands from deploy-v3.cjs output)");
  console.log("");
